...
(function ($) {
    ...
    $.fn.stackedGantt = function (options) {
        ...
        $this.update = function (data, generalMarkers, generalHighlights) {
            stackedGantt.update(data, generalMarkers, generalHighlights);
        };
        ...
        $this.getData = function () {
            return stackedGantt.getData();
        };
        $this.getGeneralMarkers = function () {
            return stackedGantt.getGeneralMarkers();
        };
        $this.getGeneralHighlights = function () {
            return stackedGantt.getGeneralHighlights();
        };
    ...
    function StackedGantt($this) {
        ...
        this.config = function (options) {
            ...
            if (style) {
                ...
                if (style.showTimeOnHeader !== undefined) showTimeOnHeader = style.showTimeOnHeader;
        ...
        function createContainers() {
            ...
            clientHeight = clientHeight ? clientHeight - 30 * (showDateOnHeader === true) - 30 * (showTimeOnHeader === true) - 20 : undefined;
            ...
            $descriptionsContainer.scroll(function () {
                $valuesContainer.scrollTop($descriptionsContainer.scrollTop());
            });

            $headerContainer = $('<div>', {
                class: 'sg_header_container',
                css: {
                    'margin-left': descriptionContainerWidth,
                },
            });
        ...
        function defineLimits() {
            ...
            var begins = rowsLimits
                ...
                .concat(
                    generalHighlights.map(function (highlight) {
            ...
            if (!beginDate && autoAdjustLimits && (begin.getMinutes() <= 10 || begin.getMinutes() >= 50)) begin.setHours(begin.getHours() - 1);
            ...
            if (!endDate && autoAdjustLimits && (end.getMinutes() <= 10 || end.getMinutes() >= 50)) end.setHours(end.getHours() + 1);
        ...
        function getRowBegin(row) {
            ...
            if (row.activities) {
                begins = begins.concat(
                    row.activities.map(function (activity) {
            ...
            if (row.markers) {
                begins = begins.concat(
                    row.markers.map(function (marker) {
                        return marker.when;
                    })
                );
            }
            ...
            if (row.thresholds) {
                begins = begins.concat(
                    row.thresholds.map(function (threshold) {
        ...
        function createHeader() {
            ...
            $headerContainer.append($timeHeaderContainer);

            hours = getHoursBetweenLimits();
            hoursQuantity = hours.length;

            hours.forEach(function (hour, hourIndex) {
                if (showTimeOnHeader) createTimeHeader(hour);

                if (showDateOnHeader && (hourIndex === 0 || hour.getHours() === 0) && !checkHeaderConflict(hourWidth * hourIndex)) dateHeaders.push(createDateHeader(hour, hourIndex));
            });
        }
        ...
        function createTimeHeader(hour) {
            ...
            $headerItem.html(('0' + hour.getHours()).slice(-2));
        ...
        function getHoursBetweenLimits() {
            ...
            for (var hour = new Date(limits.begin); hour <= limits.end; hour.setHours(hour.getHours() + 1)) {
        ...
        function createActivity(activity, $rowValueContainer, row) {
            ...
            var image = getActivityImage(activity); // activity 이미지 추가 - custom
            ...
            if (activityClick) {
                $activity.click(function () {
                    activityClick(activity, row, data);
                });
        ...
        function getActivityTimeDescription(activity) {
            return getDateDescription(activity.begin, true) + ' — ' + getDateDescription(activity.end, !isSameDay(activity.begin, activity.end));
        }
        ...
        function createGeneralMarker(marker) {
            ...
            var $marker = $('<div>', { class: 'sg_general_marker', css: css });
        ...
        function createGeneralHighlight(highlight) {
            ...
            var $highlight = $('<div>', { class: 'sg_general_highlight', css: css });
        ...
        // function appendTooltipContent($tooltip, title, descriptions, badge, subtitle, badgeBGColor, badgeFontColor, squareBadge) {
        //     if (badge) {
        //         var $badge = $('<div>', { class: 'sg_tooltip_badge', html: badge, css: { backgroundColor: badgeBGColor, color: badgeFontColor } });
        ...
        //     if (descriptions) {
        //         if (descriptions.constructor !== Array) descriptions = [descriptions];
